...
// Function to update URL based on current post index
const updateUrlForPost = useCallback((index: number) => {
  // Only update URL if not in a blob: protocol environment
...
// Effect to read URL on initial load and update URL on post change
useEffect(() => {
  ...
  const handlePopState = (event: PopStateEvent) => {
    // Only handle popstate if not in a blob: protocol environment
    if (window.location.protocol !== 'blob:') {
      if (event.state && typeof event.state.blogIndex === 'number') {
        setCurrentPostIndex(event.state.blogIndex);
      } else {
        // If state is null (e.g., initial page load without specific state), reset to 0
        setCurrentPostIndex(0);
      }
    } else {
      console.warn("Skipping popstate handling: Cannot use popstate with blob: protocol.");
    }
  };


  // Add event listener only if not in blob: protocol
  if (window.location.protocol !== 'blob:') {
  ...
  return () => {
    // Remove event listener only if it was added
    if (window.location.protocol !== 'blob:') {
...
// Effect to update URL whenever currentPostIndex changes
useEffect(() => {
  updateUrlForPost(currentPostIndex);
}, [currentPostIndex, updateUrlForPost]);
    ...
    {showBlogContentFrame && (
      ...
      >
        ...
        <div className="overflow-y-auto h-full w-full pt-16 pb-8 custom-scrollbar"> {/* Added custom-scrollbar class */}
          ...
          <h2
            className="text-black mb-8 leading-tight text-sm md:text-lg lg:text-xl" // Increased mb from 6 to 8
            style={{
          ...
          <p
            className="text-black mb-4 text-xs md:text-sm" // Decreased mb from 8 to 4
            style={{
